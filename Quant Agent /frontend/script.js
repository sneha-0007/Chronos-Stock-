...
function symbolFromBank(b) {
    return {hdfc:'HDFCBANK.NS',sbi:'SBIN.NS',icici:'ICICIBANK.NS',
            axis:'AXISBANK.NS',kotak:'KOTAKBANK.NS'}[b];
}
...
function buildPredictedFromActual(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        const d = data[i];
        let predicted;
        if (d.sma && d.sma !== 'null' && !isNaN(d.sma) && d.sma > 0) {
            const trend = i > 0 ? (data[i].close - data[i-1].close) * 0.3 : 0;
            predicted   = +(d.sma + trend).toFixed(2);
        } else {
            predicted = d.close;
        }
        result.push({ time: d.time, value: predicted });
    }
    if (data.length >= 2) {
        const step    = data[data.length-1].time - data[data.length-2].time;
        const last    = data[data.length-1];
        const slope   = (last.close - data[Math.max(0, data.length-5)].close) / 5;
        const lastSMA = (last.sma && last.sma !== 'null') ? last.sma : last.close;
        for (let f = 1; f <= 3; f++) {
            result.push({
                time:  last.time + step * f,
                value: +(lastSMA + slope * f).toFixed(2),
            });
        }
        globalPredicted = result[result.length-1].value;
    }
    return result;
}
